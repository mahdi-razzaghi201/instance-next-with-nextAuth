...
export default function generator(plop: PlopTypes.NodePlopAPI): void {
  ...
  plop.setGenerator("api", {
    ...
    actions(data) {
      ...
      const isCRUD = data.httpMethodChoice === "CRUD";
      ...
      selectedMethods.forEach((method) => {
        let type = data.type;
        if (isCRUD) {
          type = method === "get" ? "useQuery" : "useMutation";
        }

        actions.push(
          {
            type: "add",
            path: `src/api/services/{{service}}/{{path}}/${method}/${method}-{{kebabCase name}}.schema.ts`,
            templateFile: `templates/schema.${type}.hbs`,
            data: { method, type },
          },
          {
            type: "add",
            path: `src/api/services/{{service}}/{{path}}/${method}/use-${method}-{{kebabCase name}}.ts`,
            templateFile: `templates/hook.${type}.hbs`,
            data: { method, type },
          },
          {
            type: "add",
            path: `src/api/services/{{service}}/{{path}}/${method}/${method}-{{kebabCase name}}.ts`,
            templateFile: `templates/function.${type}.hbs`,
            data: { method, type },
          },
          {
            type: "add",
            path: `src/api/services/{{service}}/{{path}}/${method}/${method}-{{kebabCase name}}.types.ts`,
